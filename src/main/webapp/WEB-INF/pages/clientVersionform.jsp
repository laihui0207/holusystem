...
<form:form commandName="clientVersion" method="post" action="clientVersionform" cssClass="well"
           id="clientVersionForm" onsubmit="return validateClientVersion(this)">
<form:hidden path="id"/>
    <spring:bind path="clientVersion.QRCode">
    <div class="form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
    </spring:bind>
        <appfuse:label key="clientVersion.QRCode" styleClass="control-label"/>
        <form:input cssClass="form-control" path="QRCode" id="QRCode"  maxlength="255"/>
        <form:errors path="QRCode" cssClass="help-block"/>
    </div>
    <spring:bind path="clientVersion.clientType">
    <div class="form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
    </spring:bind>
        <appfuse:label key="clientVersion.clientType" styleClass="control-label"/>
        <form:input cssClass="form-control" path="clientType" id="clientType"  maxlength="255"/>
        <form:errors path="clientType" cssClass="help-block"/>
    </div>
    <spring:bind path="clientVersion.clientUrl">
    <div class="form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
    </spring:bind>
        <appfuse:label key="clientVersion.clientUrl" styleClass="control-label"/>
        <form:input cssClass="form-control" path="clientUrl" id="clientUrl"  maxlength="255"/>
        <form:errors path="clientUrl" cssClass="help-block"/>
    </div>
    <spring:bind path="clientVersion.createTime">
    <div class="form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
    </spring:bind>
        <appfuse:label key="clientVersion.createTime" styleClass="control-label"/>
    ...
    <spring:bind path="clientVersion.storePath">
    <div class="form-group${(not empty status.errorMessage) ? ' has-error' : ''}">
    </spring:bind>
        <appfuse:label key="clientVersion.storePath" styleClass="control-label"/>
